...
// validate fields
/* empty($title) || empty($type) || || empty($uri) || empty($description)  */
if(empty($address) || empty($owner_name) || empty($owner_email)) {
  ...
  // normal mode enabled, save new data to local db
	...
	$type ='startup'; //Start Auto
    $insert = mysql_query("INSERT INTO places (approved, title, type, address, uri, description, owner_name, owner_email) VALUES (1, '$title', '$type', '$address', '$uri', '$description', '$owner_name', '$owner_email')") or die(mysql_error());
